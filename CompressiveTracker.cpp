	...
	for (int i=0; i<featureNum; i++)
	{
		for (int j=0; j<sampleBoxSize; j++)
		{
			...
			for (size_t k=0; k<features[i].size(); k++)
			{
                ...
                float tmp = _imageIntegral.at<float>(yMin, xMin);
                tmp += _imageIntegral.at<float>(yMax, xMax); 
                tmp -= _imageIntegral.at<float>(yMin, xMax);
				tmp -= _imageIntegral.at<float>(yMax, xMin);
				tempValue *= tmp;
